...
#KPI 1 - What is the total amount each customer spent at the restaurant?
...
#KPI 2 - How many days has each customer visited the restaurant?
SELECT s.customer_id, COUNT(DISTINCT s.order_date) AS days_visited
FROM dannys_diner.sales s
GROUP BY s.customer_id;

#KPI 3 - What was the first item from the menu purchased by each customer?
...
INNER JOIN dannys_diner.sales s ON s.customer_id = cfp.customer_id
AND cfp.first_purchase_date = s.order_date
INNER JOIN dannys_diner.menu m on m.product_id = s.product_id;

#KPI 4 - What is the most purchased item on the menu and how many times was it purchased by all customers?
...
#KPI 5 - Which item was the most popular for each customer?
...
#KPI 6 - Which item was purchased first by the customer after they became a member?
...
JOIN dannys_diner.sales s ON fpam.customer_id = s.customer_id 
AND fpam.first_purchase_date = s.order_date
JOIN dannys_diner.menu m ON s.product_id = m.product_id;

#KPI 7 - Which item was purchased just before the customer became a member?
...
JOIN dannys_diner.sales s ON lpbm.customer_id = s.customer_id 
AND lpbm.last_purchase_date = s.order_date
JOIN dannys_diner.menu m ON s.product_id = m.product_id;

#KPI 8 - What is the total items and amount spent for each member before they became a member?
...
#KPI 9 - If each $1 spent equates to 10 points and sushi has a 2x points multiplier - how many points would each customer have?
...
#KPI 10 - In the first week after a customer joins the program (including their join date) they earn 2x points on all items, not just sushi - how many points do customer A and B have at the end of January?*/
SELECT 
    s.customer_id, 
    SUM(
        CASE 
...
#KPI 11 - Recreate the table output using the available data
